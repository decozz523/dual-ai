...
const authPrivacyLink = $("authPrivacyLink");
const authTermsLink = $("authTermsLink");
...
function updateAuthConsentState() {
  const isChecked = !!consentCheckbox?.checked;
  if (authSignInBtn) authSignInBtn.disabled = !isChecked;
  if (authSignUpBtn) authSignUpBtn.disabled = !isChecked;
  if (!isChecked) {
    setAuthMessage("Подтвердите согласие с политикой и условиями.", "error");
  } else if (authMessageEl.classList.contains("error") && authMessageEl.textContent.includes("согласие")) {
    setAuthMessage("");
  }
}
...
consentCheckbox?.addEventListener("change", updateAuthConsentState);
authPrivacyLink?.addEventListener("click", () => {
  openPrivacyModal();
});
authTermsLink?.addEventListener("click", () => {
  openTermsModal();
});
